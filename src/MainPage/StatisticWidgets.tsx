...
export const StatisticWidgets: FC<{ fromDate: number; toDate: number; history: any[]; timeframeLabelMap: any }> = ({
  fromDate,
  toDate,
  history,
  timeframeLabelMap,
  ...
  const filteredHistory = history.filter(
    (c) => moment(c.limitTrade?.date).unix() >= fromDate && moment(c.limitTrade?.date).unix() <= toDate,
  );

  const timeframes = useMemo(
    () => Array.from(new Set(filteredHistory.filter((p) => p.timeframe).map((p) => p.timeframe))).sort((a, b) => a - b),
    [filteredHistory],
  );

  const names = useMemo(
    () => Array.from(new Set(filteredHistory.filter((p) => p.pattern).map((p) => p.pattern))).sort((a, b) => a - b),
    [filteredHistory],
  );

  const NametotalPnL = useMemo(
    () =>
      filteredHistory
        .filter((p) => p.PnL)
        .reduce((acc, curr) => {
