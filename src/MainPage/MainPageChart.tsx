...
export const MainPageChart: FC<{
  ...
  const _markers: SeriesMarker<Time>[] = useMemo(
    () =>
      [
        tradesOrdernoMap[selectedPattern?.limitOrderNumber],
        tradesMap[selectedPattern?.stopTradeId],
        tradesMap[selectedPattern?.takeTradeId],
      ]
        .filter(Boolean)
        .map((t) => ({
  ...
  return (
    width > 1200 && (
      <Col span={8}>
        <Chart
          {...props}
          data={candles.filter((candle) => !notTradingTime(candle))}
