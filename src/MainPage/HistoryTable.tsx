...
import { moneyFormat, summ } from './MainPage';
...
export const HistoryTable: FC<{
  ...
  const historyTableData = useMemo(() => {
    ...
    for (let i = 0; i < data.length; i++) {
      const currentDay = moment(data[i].limitTrade?.date).format('YYYY-MM-DD');
      if (!dayPositionsWithSummaryMap[currentDay]) {
        const currentDayPositions = data.filter((p) => moment(p.limitTrade?.date).format('YYYY-MM-DD') === currentDay);
    ...
    <>
      ...
      <Table
        ...
        onRow={(record) => {
          return {
            style:
              getPatternKey(selectedPattern) === getPatternKey(record)
                ? { backgroundColor: 'rgba(179, 199, 219, .2)' }
                : record.PnL < 0
                  ? {
