...
const MainPage: React.FC = () => {
  ...
  const getPatternKey = useCallback(
    (pattern) => (pattern?.type === 'summary' ? pattern.openDate : `${pattern?.ticker}_${pattern?.timeframe}_${pattern?.liquidSweepTime}`),
    [],
  );
  ...
  const ordersMap = useMemo(
    () =>
      portfolio?.orders?.reduce((acc, curr) => {
        acc[curr.id] = curr;
        return acc;
      }, {}) || {},
    [portfolio?.orders],
  ...
  const stopordersMap = useMemo(
    () =>
      portfolio?.stoporders?.reduce((acc, curr) => {
        acc[curr.id] = curr;
        return acc;
      }, {}) || {},
  ...
  const tradesMap = useMemo(
    () =>
      portfolio?.trades?.reduce((acc, curr) => {
        acc[curr.id] = curr;
        return acc;
      }, {}) || {},
    [portfolio?.trades],
  ...
  const accTradesOrdernoQtyMap = useMemo(
    () =>
      portfolio?.trades?.reduce((acc, curr) => {
        if (!acc[curr.orderno]) {
          acc[curr.orderno] = 0;
  ...
  const tradesOrdernoMap = useMemo(
    () =>
      portfolio?.trades?.reduce((acc, curr) => {
  ...
  const positions = useMemo(
    () => patterns.filter((p) => !p.takeTradeId && !p.stopTradeId && p.limitTradeId && p.stopLoss?.status === 'working'),
    [patterns],
  );
  ...
  const history = useMemo(
    () =>
      patterns
        .filter((p) => {
          const baseBool =
            (!selectedTicker || p.ticker === selectedTicker) &&
          ...
          let percent =
            p.limitTrade?.price > value?.stopPrice ? p.limitTrade?.price / value?.stopPrice : value?.stopPrice / p.limitTrade?.price;
        ...
        .map((row) => ({
          ...
          PnL:
            row.limitTrade?.side === 'buy'
              ? accTradesOrdernoQtyMap[row.limitTrade?.orderno] *
                  ((row.stopLossTrade?.price || row.takeProfitTrade?.price) - row.limitTrade?.price) -
                calculateProdPositionFee(row)
              : row.limitTrade?.side === 'sell'
                ? accTradesOrdernoQtyMap[row.limitTrade?.orderno] *
                    (row.limitTrade?.price - (row.stopLossTrade?.price || row.takeProfitTrade?.price)) -
                  calculateProdPositionFee(row)
                : undefined,
        }))
        .sort((a, b) => b.limitTrade?.date.localeCompare(a.limitTrade?.date)),
    [stopFrom, stopTo, selectedName, selectedTicker, selectedTF, accTradesOrdernoQtyMap, patterns],
  );
  ...
  const items: TabsProps['items'] = [
    ...
    },
    {
      key: 'orders',
      label: 'Заявки',
      children: <OrdersTable ordersMap={ordersMap} orders={orders} pageSize={pageSize} onSelect={onSelect} />,
    },
    {
      key: 'history',
  ...
  const TabExtra = () => {
    const tfOptions = useMemo(
      () =>
        Object.entries(timeframeLabelMap).map(([value, label]) => ({
          value,
          label,
        })),
      [],
    ...
    const nameOptions = useMemo(
      () =>
        Array.from(new Set(history.map((h) => h.pattern))).map((value) => ({
          value,
          label: value,
        })),
      [],
    ...
    <Space direction="vertical" style={{ width: '100%' }}>
      ...
      <Row gutter={8}>
        ...
        <MainPageChart
          ...
          candles={candles}
          tradesMap={tradesMap}
          tradesOrdernoMap={tradesOrdernoMap}
        />
      </Row>
    </Space>
  );
