...
const baseQuery = () =>
  fetchBaseQuery({
    ...
    baseUrl: 'https://176.114.69.4/tinkoff',
    prepareHeaders: (headers, { getState }: any) => {
      const token = getState().alorSlice.tToken;

      if (token) {
        headers.set('x-tinkoff-token', token);
      }
...
export const baseQueryTinkoffWithReauth = async (args: string | FetchArgs, api: BaseQueryApi, extraOptions) => {
