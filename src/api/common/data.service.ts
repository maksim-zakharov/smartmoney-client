...
export class DataService {
  ...
  getChartData(ticker: string, resolution: ResolutionString, periodParams: PeriodParams) {
    ...
    } else if (ticker.includes('XT:')) {
      const _ticker = ticker.split('XT:')[1];
      request$ = from(
        fetch(
          `${this.cryptoUrl}/xt/candles?tf=${this.parseTimeframe(resolution)}&from=${Math.max(periodParams.from, 0)}&symbol=${_ticker}&to=${Math.max(periodParams.to, 1)}`,
        ).then((res) => {
          if (!res.ok) {
            throw new Error(`HTTP error! status: ${res.status}`);
          }
