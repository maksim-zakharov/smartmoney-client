...
import { TLineSeries } from './TestChart';
...
const ChartFC: FC<Props> = ({
  ...
  const filteredLineSerieses = useMemo(() => {
    ...
    return lineSerieses
      .filter((series) => {
        const pointsInRange = series.data.filter((d) => d.time >= visibleRange.from && d.time <= visibleRange.to);
      ...
      .map((series) => {
        ...
        return {
          ...series,
          data: [...(minPoint ? [minPoint] : []), ...pointsInRange, ...(maxPoint ? [maxPoint] : [])],
        };
      });
  ...
  const filteredMarkers = useMemo(() => {
    ...
    return markers.filter((m) => m.time >= visibleRange.from && m.time <= visibleRange.to);
  ...
  useEffect(() => {
    ...
    const toRemove = prevPrimitives.filter((p) => !primitives.includes(p));

    // Находим примитивы для добавления
    const toAdd = primitives.filter((p) => !prevPrimitives.includes(p));
