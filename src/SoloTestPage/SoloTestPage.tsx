...
export const SoloTestPage = () => {
    ...
    const checkboxValues = new Set((searchParams.get('checkboxes') || "tradeOB,BOS,swings,showEndOB,showHiddenSwings,showPositions").split(','));
    ...
    const {swings, trend, boses, orderBlocks, positions} = useMemo(() => {
        ...
        if (config.tradeOB) {
            const fakeoutPositions = iterationCalculatePositions(data, swings as any, canTradeOrderBlocks, maxDiff, multiStop, stopPaddingPercent);
    ...
    return <Layout style={{display: 'flex', flexDirection: 'row', gap: '8px'}}>
        <Sider width={280} style={{padding: 16, height: 'calc(100vh - 84px)', overflow: 'auto', overflowX: 'hidden'}}>
            ...
            <Checkbox.Group onChange={setCheckboxValues} value={Array.from(checkboxValues)}
                ...
                <Checkbox key="tradeCHoCHWithIDM" value="tradeCHoCHWithIDM">Торговать CHoCHWithIDM</Checkbox>
                <Checkbox key="tradeFlipWithIDM" value="tradeFlipWithIDM">Торговать FlipWithIDM</Checkbox>
                <Checkbox key="tradeOBEXT" value="tradeOBEXT">Торговать OBEXT</Checkbox>
