...
import { PlusClickParams } from '../../assets/charting_library';
...
import { addAlert } from '../../api/alerts.slice';
...
export const StatArbPage = ({
  ...
  const [openPlusDialog, setOpenPlusDialog] = useState(false);
  ...
  const handlePlusClick = (params: PlusClickParams) => {
    setOpenPlusDialog(true);
    form.setValue('ticker', params.symbol);
    form.setValue('price', Number(params.price.toFixed(5)));
  };

  const handleClose = () => {
    setOpenPlusDialog(false);
  };
      ...
      <div className="relative" style={{ height }}>
        {isVisible && (
          <>
            ...
            {/*{!isSecondForex ? (*/}
            <TWChart
              ticker={`${tickerStock}/${tickerFuture}`}
              height={height}
              multiple={multiple}
              small={onlyChart}
              onPlusClick={handlePlusClick}
            />
            <Dialog open={openPlusDialog}>
