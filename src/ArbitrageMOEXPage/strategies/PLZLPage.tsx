...
export const PLZLPage = () => {
  ...
  const [diff, setDiff] = useState<number>(0.065);
  const tickerStock = 'GOLD-6.25';
  const tickerFuture = 'PLZL';
  ...
  const avg = 0.85;
  const sellLineDataX2 = useMemo(() => stockData.map((s) => avg + diff * 2), [stockData, diff]);
  ...
  const buyLineDataX2 = useMemo(() => stockData.map((s) => avg - diff * 2), [stockData, diff]);
